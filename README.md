...
1. **降低解析度**：減少 `FRAME_WIDTH` 和 `FRAME_HEIGHT` 可提升處理速度
2. **簡化模型**：將 `MODEL_COMPLEXITY` 設為 0（預設值）
3. **關閉不必要的處理**：若不需要馬賽克功能，可將 `BLACKLIST_GESTURES` 設為空元組 `()`

## 注意事項

1. 本系統的手勢識別基於手指角度，在不同角度和距離下可能有所差異
2. 光線條件會影響識別準確度，建議在光線充足的環境下使用
3. 馬賽克功能主要用於展示和教育目的，實際應用需根據情境調整
4. 建議在使用前測試各種手勢，了解系統的識別特性
